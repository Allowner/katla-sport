...
export class HiveSectionService {
  ...
  getItems(hiveSectionId: number): Observable<Array<HiveSectionProductListItem>> {
    return this.http.get<Array<HiveSectionProductListItem>>(`${this.url}${hiveSectionId}/items`);
  }
  ...
  setHiveSectionStatus(hiveSectionId: number, deletedStatus: boolean): Observable<Object> {
    return this.http.put(`${this.url}${hiveSectionId}/status/${deletedStatus}`, deletedStatus);
  }
}
